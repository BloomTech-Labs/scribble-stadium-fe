import { global, faceoffs } from '../state/actions';
import { reducer } from '../state/reducers/faceoffsReducer';

describe('faceoffsReducer test suite', () => {
  ...
  it('should return an array if passed in action', () => {
    const action = {
      type: faceoffs.SET_SQUAD_FACEOFFS,
      payload: ['a', 'b', 'c', 'd'],
    };
    const state = reducer(initialState, action);
